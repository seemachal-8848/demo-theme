                  ...
                  return (
                    <div
                      ...
                      onClick={() => {
                        ...
                        const currentPending = newPendingSelections[section] || [];
                        const willBeSelected = !isSelected;
                        newPendingSelections[section] = willBeSelected
                          ? [...currentPending, val]
                          : currentPending.filter(v => v !== val);

                        let filterUpdates = [valueToPass];
                        ...
                        if (isVehicleSection) {
                          ...
                          const selectedInVehicleCompany =
                            selectedFilters.find((f: any) => f.name === vehicleCompanySection)?.value || [];
                          const pendingInVehicleCompany = newPendingSelections[vehicleCompanySection] || [];

                          if (willBeSelected) {
                            // Selecting a vehicle: Auto-select the vehicle company if not already selected
                            if (
                              !selectedInVehicleCompany.includes(vehicleCompany) &&
                              !pendingInVehicleCompany.includes(vehicleCompany)
                            ) {
                              newPendingSelections[vehicleCompanySection] = [
                                ...(newPendingSelections[vehicleCompanySection] || []),
                                vehicleCompany
                              ];
                              filterUpdates.push({
                              ...
                              setSelectionOrder(prev => {
                                ...
                                const newSelection = [
                                  vehicleCompany,
                                  ...current.filter(v => v !== vehicleCompany)
                                ];
                                return { ...prev, [vehicleCompanySection]: newSelection };
                              });
                            }
                          } else {
                            // Deselecting a vehicle: Check if any other vehicles of this company are selected
                            const vehicleSection = filtersData.filters.find(
                              (f: any) => f.section === "Vehicle"
                            );
                            const selectedVehicles =
                              selectedFilters.find((f: any) => f.name === "Vehicle")?.value || [];
                            const pendingVehicles = newPendingSelections["Vehicle"] || [];

                            // Get all currently selected vehicles (excluding the one being deselected)
                            const allSelectedVehicles = [...selectedVehicles, ...pendingVehicles].filter(
                              (v: string) => v !== val
                            );

                            // Check if any remaining selected vehicles belong to this vehicle company
                            const hasOtherVehicles = vehicleSection?.values.some(
                              (vehicle: any) =>
                                vehicle.vehicle_company === vehicleCompany &&
                                allSelectedVehicles.includes(vehicle.vehicle_name)
                            );

                            if (!hasOtherVehicles) {
                              // No other vehicles of this company are selected, deselect the vehicle company
                              newPendingSelections[vehicleCompanySection] = (
                                newPendingSelections[vehicleCompanySection] || []
                              ).filter((v: string) => v !== vehicleCompany);
                              filterUpdates.push({
                                label: vehicleCompany,
                                value: vehicleCompany,
                                section: vehicleCompanySection
                              ...
                              setSelectionOrder(prev => {
                                const current = prev[vehicleCompanySection] || [];
                                const newSelection = current.filter((v: string) => v !== vehicleCompany);
                                return { ...prev, [vehicleCompanySection]: newSelection };
                              });
                            }
                          }
                        ...
                        // Apply the pendingSelections update
                        setPendingSelections(newPendingSelections);

                        // Update selection order for the current section
                        setSelectionOrder(prev => {
                          const current = prev[section] || [];
                          const newSelection = willBeSelected
                            ? [val, ...current.filter(v => v !== val)]
                            : current.filter(v => v !== val);
                          return { ...prev, [section]: newSelection };
                        });
                        ...
                        handleFilterSelectDropDown(filterUpdates, { name: section });
                      }}
                    >
                      {val}
                    </div>
